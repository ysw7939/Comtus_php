<?php if( !defined('BASEPATH') )
{
    ...
    class Data_validation
    {
        ...
        private static $_validation_data        = [];
        private static $_validation_field_data  = [];
        private static $_validation_error_array = [];
        private        $_validation_lang_file   = 'data_validation';
        ...
        public function clear()
        {
            ...
            self::$_validation_field_data  = [];
            self::$_validation_error_array = [];
        }
         ...
         * @return    self
         */
        public function set_rules($field, $label = '', $rules = '', $callback_params = [])
        {
            ...
            self::$_validation_field_data[$field] = [
                'field'           => $field,
                'label'           => $label,
                'rules'           => $rules,
         ...
         * @return    data
         */
        public function validate($data = [], $strict_mode = TRUE)
        {

            if( $strict_mode && count(self::$_validation_field_data) == 0 )
            {
                throw new Validation_Exception('No data validation rules have been set. (running in strict mode)');
            }
            ...
            foreach( self::$_validation_field_data as $field => $row )
            {
                ...
                {
                    // TO DO : field 데이터가 0과 같은 integer일 경우 blank("")와 같은것으로 판단하여 field 데이터를 NULL로 설정됨.
                    // 그로인해 required룰이 적용될 경우 예외를 발생시키는 버그 발생.
                    // 그래서 조건을 '!='에서 '!=='으로 변경
        ...
        function min_length($str, $val)
        {
            ...
            {
                return ( mb_strlen($str, "EUC-KR") < $val ) ? FALSE : TRUE;
            }
        ...
        function valid_birth($str)
        {
            return ( !preg_match("/^(19[0-9][0-9]|20\d{2})(0[0-9]|1[0-2])(0[1-9]|[1-2][0-9]|3[0-1])$/", $str) ) ? FALSE : TRUE;
        }
        ...
        /**
         * @author 언틸(until)
         * @date 2021-01-12 오후 3:14
         * @description 휴대폰 번호 검사
         * @param $str
         *
         * @return bool
         */
        ...
        {
            return ( !preg_match("/^(010|011|016|017|018|019)[0-9]{3,4}[0-9]{4}/", $str) ) ? FALSE : TRUE;
        }
         ...
         * <pre>
         *
         * 전준형(midtics), 2015. 8. 12.
         * 1. 개요 : API TAG validation용 함수 추가
         * 2. 설명 : A000234 형태의 API
         *
         * </pre>
         * @param unknown $str
         *
         * @return boolean
         */
        function valid_api_tag($str)
        {
            return ( !preg_match("/^([A-Z]{1}[0-9]{6})$/i", $str) ) ? FALSE : TRUE;
        }
         ...
         * ============================================
         * 1.0            || 2018. 10. 30.        || 최초 작성
         * ============================================
         * @Method  설명 : 한글,영어,숫자 가 아닌 다른 문자가 포함되면 false
         *
         * @param   :
         *
         * @return  : boolean
         * @example :
         */
        function valid_id_kr($str)
        {
            return ( !preg_match("/^([가-힣a-z0-9_])+$/i", $str) ) ? FALSE : TRUE;
        }
        ...
        function date_kr($str)
        {
            $date_array = explode("/", $str); // split the array

            if( count($date_array) == 3 )
            {
                $yy = $date_array[0]; // first element is year
                $mm = $date_array[1]; // second element of the array is month
                $dd = $date_array[2]; // third element is date
         ...
         * 파일 형식 검증
         * @author 언틸(until)
         * @date 2021-03-02 오전 10:40
         * @param $value
         * @param $list
         *
         * @return bool
         */
        function in_mime($value, $list)
        {
            $value = $value['name'];

            $mime = explode('.', $value);

            if( count($mime) > 1 ) $mime = $mime[1];
            else $mime = $value;

            $mime = strtolower($mime);
            return in_array($mime, explode(',', $list), TRUE);
        }

        /**
         * @description
         * @author 언틸(until)
         * @date 2021-03-02 오전 11:51
         * @param $value
         * @param $size
         *
         * @return bool
         */
        function max_size($value, $size)
        {
            $file_size = $value['size'] / 1024; // KB로 변환
         ...
         * @return    void
         */
        private function _execute($row, $rules, $postdata = NULL, $cycles = 0, $callback_params = [])
        {
            ...
            if( is_array($postdata) )
            {
                ...
                {
                    // TO DO : 필드데이터가 배열일 경우 배열의 Key가 단순히 인덱스가 아닐경우 오류가 발생하므로 Key값으로 전달
                    $this->_execute($row, $rules, $val, $key, $callback_params);
